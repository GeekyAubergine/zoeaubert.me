...
use crate::{
    domain::models::{raw_content::RawContent, omni_post::OmniPost},
...
pub fn parse_content_into_book_review(content: &RawContent) -> Result<Review> {
    match content {
        RawContent::MicroPost(post) => parse_markdown_into_book_review(&post.content),
        RawContent::MastodonPost(post) => parse_markdown_into_book_review(&post.content()),
