...
use crate::{
    domain::models::{source_post::SourcePost, post::Post},
...
pub fn parse_content_into_book_review(content: &SourcePost) -> Result<Review> {
    match content {
        SourcePost::MicroPost(post) => parse_markdown_into_book_review(&post.content),
        SourcePost::MastodonPost(post) => parse_markdown_into_book_review(&post.content()),
