...
use crate::{
    domain::{
        models::{raw_cosnstent::SourcePost, movie::MovieReview, post::Post},
...
pub fn parse_content_into_movie_review(post: &SourcePost) -> Result<Review> {
    match post {
        SourcePost::MicroPost(post) => parse_markdown_into_movie_review(&post.content),
        SourcePost::MastodonPost(post) => parse_markdown_into_movie_review(&post.content()),
