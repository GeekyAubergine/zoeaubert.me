...
use crate::prelude::*;
use crate::{
    domain::models::omni_post::OmniPost,
    infrastructure::query_services::micro_posts_query_service::MicroPostsQueryService,
};
...
pub async fn find_omni_posts_by_date(
    app_state: &AppState,
    ...
    if OmniPostFilterFlags::MICRO_POST.intersects(filter) {
        let micro_posts = MicroPostsQueryService::find_all_by_date(&app_state).await?;
...
pub struct OmniPostQueryService;

impl OmniPostQueryService {
    pub fn filter_all() -> OmniPostFilterFlags {
        OmniPostFilterFlags::all()
    }
